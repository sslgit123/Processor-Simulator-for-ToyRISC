    ...
    public void performOF() 
    {
        ...
        else if (IF_OF_Latch.isOF_enable()) 
        {
            ...
            else if (operation.toString().equals("jmp")) 
            {
                ...
                {
                    immediate = containingProcessor.getRegisterFile().getValue(forwardInstruction.getDestinationOperand().getValue());
                } 
                else 
                {
            ...
            while (true) 
            {
                ...
                else if (CheckConflict(forwardInstruction, OF_EX_Latch.instruction)&& OF_EX_Latch.instruction.getOperationType() != OperationType.nop) 
                {
                ...
                else if (CheckConflict(forwardInstruction, EX_MA_Latch.instruction)&& EX_MA_Latch.instruction.getOperationType() != OperationType.nop) 
                {
                    IF_EnableLatch.setIF_enable(false);
                    OF_EX_Latch.setInstruction(nop);
                    containingProcessor.setStalls(containingProcessor.getStalls() + 1);
                ...
                else if (CheckConflict(forwardInstruction, MA_RW_Latch.instruction)&& MA_RW_Latch.instruction.getOperationType() != OperationType.nop) 
                {
                    IF_EnableLatch.setIF_enable(false);
                    OF_EX_Latch.setInstruction(nop);
                    containingProcessor.setStalls(containingProcessor.getStalls() + 1);
