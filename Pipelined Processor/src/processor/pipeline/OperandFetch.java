...
public class OperandFetch {
	...
	public static boolean isConflict(Instruction instruction, int rs1, int rs2) 
	{
		...
		else if((instNumber == 6 || instNumber ==7)&&(rs1 == 31)||rs2 == 31)
		{
			return true;
		}
	}
	...
	public void performOF() 
	{
		if (IF_OF_Latch.isOF_enable()) 
		{
			...
			case sra:
				...
				if (isConflict(instruction_ex_stage, registerNo, registerNo2))
					conflict_inst = true;
				if (isConflict(instruction_ma_stage, registerNo, registerNo2))
					conflict_inst = true;
				if (isConflict(instruction_rw_stage, registerNo, registerNo2))
					conflict_inst = true;
				if (conflict_inst) 
				{
					this.ConflictBubble();
			...
			default:
				...
				registerNo = Integer.parseInt(instruction.substring(5, 10), 2);
				rs1.setValue(registerNo);
				if (isConflict(instruction_ex_stage, registerNo, registerNo)) 
				{
					conflict_inst = true;
				}	
				if (isConflict(instruction_ma_stage, registerNo, registerNo)) 
				{
					conflict_inst = true;
				}
				if (isConflict(instruction_rw_stage, registerNo, registerNo)) 
				{
					conflict_inst = true;
				}
